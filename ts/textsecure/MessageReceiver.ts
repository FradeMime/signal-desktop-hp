  ...
  implements IRequestHandler
{
  ...
  public handleRequest(request: IncomingWebSocketRequest): void {
    ...
    log.info('handlerequest incomingwebsocketrequest解密消息');
  ...
  private async handleSentMessage(
    ...
    log.info(
      `handleSentMessage消息处理:envelope${JSON.stringify(
        envelope
    ...
    log.info(`sentContainer:${JSON.stringify(sentContainer)}`);
    ...
    if (msg.flags && msg.flags & Proto.DataMessage.Flags.END_SESSION) {
      if (destinationUuid) {
        ...
        if (theirUuid) {
          ...
          log.info(`handleSentMessage: uuid not found for ${destination}`);
          p = Promise.resolve();
        }
      } else {
        throw new Error(
  ...
  private async handleDataMessage(
    ...
    log.info(
      `handleDataMsg消息处理:${JSON.stringify(envelope)};msg:${JSON.stringify(
        msg
      )}`
    );
    const logId = this.getEnvelopeId(envelope);
